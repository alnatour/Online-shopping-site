...
class ArticleDb
    ...
    const SQL_UPDATE                = "UPDATE products_tb set contact_id = ?, title = ?, price = ?, discount =?, imagee = ?, article = ?, update_datum = ?, subcategory_id=?, specification_id=? WHERE id = ?";
    ...
    function GetAllArtikelsWithSp()
    {
        
        $statement = $this->pdo->prepare(self::GET_ALL_ARTICLES);
        $statement->execute();
        $artikes = $statement->fetchAll(PDO::FETCH_ASSOC);
        return $artikes;
    }
    ...
    function getRelated($cid)
    {
        ...
        $articles = $statement->fetchAll(PDO::FETCH_CLASS, ArticleInfo::class);
        return $articles;
    }
    ...
    function GetByid($id)
    {
        ...
        $article = $statement->fetchObject(ArticleInfo::class);
        return $article;
    }
    ...
    function editArticle($artikelinfo)
    {
        ...
        $statement->execute(array(
            ...
            $artikelinfo->getSubcategoryId(),
            $artikelinfo->getSpecification_id(),
            $artikelinfo->getId()
        ));
    }
...
/*
 const COUNT_ORDER_ARTICLE        = "SELECT count(*) FROM order_products op LEFT JOIN products_tb p ON op.product_id = p.id WHERE p.id=?";
    const GET_ALL_PRODUCTS          = "SELECT *,p.id AS id, sp.id AS sp_id FROM products_tb p
    LEFT JOIN specification sp ON p.specification_id = sp.id";
    function CountOrdersArticle($pid)
    {
        $sql = self::COUNT_ORDER_ARTICLE;
        $statement = $this->pdo->prepare($sql);
     ...
     $countOrdersArticle = $articledb->CountOrdersArticle($Product['id']);
     $remainingProducts = $Product['stock'] - $countOrdersArticle;
 */
